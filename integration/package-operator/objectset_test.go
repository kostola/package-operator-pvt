...
func runObjectSetSetupPauseTeardownTest(t *testing.T, namespace, class string, testEnv *testEnv) {
	t.Helper()
	...
	cmGVK, err := apiutil.GVKForObject(cm4, testEnv.Scheme)
	...
	cm4Key := crClient.ObjectKey{
		Name: cm4.Name, Namespace: objectSet.Namespace,
	}
	cm5Key := crClient.ObjectKey{
		Name: cm5.Name, Namespace: objectSet.Namespace,
	}

	ctx := logr.NewContext(context.Background(), testr.New(t))

	require.NoError(t, testEnv.Client.Create(ctx, objectSet))
	cleanupOnSuccess(ctx, t, objectSet, testEnv)
	...
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSet, corev1alpha1.ObjectSetAvailable, metav1.ConditionFalse))
	...
	assert.NoError(t, testEnv.Waiter.WaitForObject(ctx, objectSet,
		"Waiting for .status.controllerOf to be updated",
		func(obj crClient.Object) (done bool, err error) {
	...
	require.NoError(t, testEnv.Client.Get(ctx, cm4Key, currentCM4))

	// expect cm-5 to NOT be present as Phase-1 didn't complete.
	currentCM5 := &corev1.ConfigMap{}
	require.EqualError(t, testEnv.Client.Get(ctx, cm5Key, currentCM5), `configmaps "cm-5" not found`)

	// Patch cm-4 to pass probe.
	// -------------------------
	require.NoError(t,
		testEnv.Client.Patch(ctx, currentCM4,
			crClient.RawPatch(types.MergePatchType, []byte(`{"metadata":{"annotations":{"name":"cm-4"}}}`))))

	// Expect ObjectSet to become Available.
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSet, corev1alpha1.ObjectSetAvailable, metav1.ConditionTrue))
	...
	// Pause ObjectSet.
	require.NoError(t, testEnv.Client.Patch(ctx, objectSet,
		crClient.RawPatch(types.MergePatchType, []byte(`{"spec":{"lifecycleState":"Paused"}}`))))

	// ObjectSet is Pausing...
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSet, corev1alpha1.ObjectSetPaused, metav1.ConditionTrue))

	// should be reconciled to "banana":"bread", if reconciler would not be paused.
	require.NoError(t, testEnv.Client.Patch(ctx, currentCM4,
		crClient.RawPatch(types.MergePatchType, []byte(`{"data":{"banana":"toast"}}`))))

	// Wait 5s for the object to be reconciled, which should not happen, because it's paused.
	require.EqualError(t,
		testEnv.Waiter.WaitForObject(ctx, currentCM4, "to NOT be reconciled to its desired state", func(obj crClient.Object) (done bool, err error) {
	...
	// Unpause ObjectSet.
	require.NoError(t, testEnv.Client.Patch(ctx, objectSet,
		crClient.RawPatch(types.MergePatchType, []byte(`{"spec":{"lifecycleState":"Active"}}`))))

	// ObjectSet is Unpausing...
	require.NoError(t,
		testEnv.Waiter.WaitForObject(ctx, objectSet, "to not report Paused anymore", func(obj crClient.Object) (done bool, err error) {
	...
	require.NoError(t,
		testEnv.Waiter.WaitForObject(ctx, currentCM4, "to be reconciled to its desired state", func(obj crClient.Object) (done bool, err error) {
			cm := obj.(*corev1.ConfigMap)
			return cm.Data["banana"] == "bread", nil
	...
	require.NoError(t,
		testEnv.Client.Patch(ctx, currentCM4,
			crClient.RawPatch(types.StrategicMergePatchType, []byte(`{"metadata":{"finalizers":["package-operator.run/test-blocker"]}}`))))

	// Archive ObjectSet to start teardown.
	require.NoError(t, testEnv.Client.Patch(ctx, objectSet,
		crClient.RawPatch(types.MergePatchType, []byte(`{"spec":{"lifecycleState":"Archived"}}`))))

	// ObjectSet is Archiving...
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSet, corev1alpha1.ObjectSetArchived, metav1.ConditionFalse))

	// expect cm-5 to be gone already.
	require.EqualError(t, testEnv.Client.Get(ctx, crClient.ObjectKey{
		Name: cm5.Name, Namespace: objectSet.Namespace,
	...
	require.NoError(t,
		testEnv.Client.Patch(ctx, currentCM4,
			crClient.RawPatch(types.JSONPatchType, []byte(`[{"op":"remove","path":"/metadata/finalizers/0" }]`))))

	// ObjectSet is now Archived.
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSet, corev1alpha1.ObjectSetArchived, metav1.ConditionTrue))
...
func TestObjectSet_setupPauseTeardown(t *testing.T) {
	...
	for i := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			runObjectSetSetupPauseTeardownTest(t, "default", test.class, testEnv)
		})
	}
}
...
func runObjectSetHandoverTest(t *testing.T, namespace, class string, testEnv *testEnv) {
	t.Helper()
	...
	cmGVK, err := apiutil.GVKForObject(cm1, testEnv.Scheme)
	require.NoError(t, err)
	...
	require.NoError(t, testEnv.Client.Create(ctx, objectSetRev1))
	cleanupOnSuccess(ctx, t, objectSetRev1, testEnv)

	// Expect ObjectSet Rev1 to become Available.
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSetRev1, corev1alpha1.ObjectSetAvailable, metav1.ConditionTrue))
	...
	// Create Revision 2
	require.NoError(t, testEnv.Client.Create(ctx, objectSetRev2))
	cleanupOnSuccess(ctx, t, objectSetRev2, testEnv)

	// Expect ObjectSet Rev2 report not Available, due to failing probes on cm-1.
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSetRev2, corev1alpha1.ObjectSetAvailable, metav1.ConditionFalse))
	...
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSetRev1, corev1alpha1.ObjectSetInTransition, metav1.ConditionTrue))

	// expect only cm-2 to be reported under "ControllerOf" in revision 1
	require.NoError(t, testEnv.Client.Get(ctx, crClient.ObjectKeyFromObject(objectSetRev1), objectSetRev1))
	require.Equal(t, []corev1alpha1.ControlledObjectReference{
		{
			Kind:      "ConfigMap",
			Name:      currentCM2.Name,
	...
	assert.NoError(t, testEnv.Waiter.WaitForObject(ctx, objectSetRev2,
		"Waiting for .status.controllerOf to be updated",
		func(obj crClient.Object) (done bool, err error) {
	...
	require.NoError(t,
		testEnv.Client.Patch(ctx, currentCM1,
			crClient.RawPatch(types.MergePatchType, []byte(`{"metadata":{"annotations":{"name":"cm-1"}}}`))))

	// Expect ObjectSet Rev2 to become Available.
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSetRev2, corev1alpha1.ObjectSetAvailable, metav1.ConditionTrue))

	// Archive ObjectSet Rev1
	require.NoError(t, testEnv.Client.Patch(ctx, objectSetRev1,
		crClient.RawPatch(types.MergePatchType, []byte(`{"spec":{"lifecycleState":"Archived"}}`))))
	require.NoError(t,
		testEnv.Waiter.WaitForCondition(ctx, objectSetRev1, corev1alpha1.ObjectSetArchived, metav1.ConditionTrue))
...
func TestObjectSet_handover(t *testing.T) {
	...
	for i := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			runObjectSetHandoverTest(t, "default", test.class, testEnv)
		})
	}
}

func cleanupOnSuccess(ctx context.Context, t *testing.T, obj crClient.Object, testEnv *testEnv) {
	t.Helper()
	t.Cleanup(func() {
		if !t.Failed() {
			// Make sure objects are completely gone before closing the test.
			_ = testEnv.Client.Delete(ctx, obj, crClient.PropagationPolicy(metav1.DeletePropagationForeground))
			_ = testEnv.Waiter.WaitToBeGone(ctx, obj, func(obj crClient.Object) (done bool, err error) { return false, nil })
		}
	})
}
...
func assertControllerNameHasPrefix(t *testing.T, ownerNamePrefix string, obj crClient.Object) {
	t.Helper()
	found := controllerNameHasPrefix(ownerNamePrefix, obj)

	require.True(t, found, "controller name of %s not prefixed with %s", crClient.ObjectKeyFromObject(obj), ownerNamePrefix)
}

func controllerNameHasPrefix(ownerNamePrefix string, obj crClient.Object) bool {
...
func getOwnerRefsFromAnnotations(obj crClient.Object) []metav1.OwnerReference {
